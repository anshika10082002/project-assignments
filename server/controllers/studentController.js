...
const registerStudent= async function(req,res){
  try{
         ...
         //|| !isValidName(name)
         if(!name ){return res.status(400).send({status:false,message:"please provide correct name"})}
...
const loginStudent= async function(req,res){
    try{
         ...
         let findStudent= await studentModel.findOne({email:email,password:password})
